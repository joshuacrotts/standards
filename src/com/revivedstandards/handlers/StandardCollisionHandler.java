...
public class StandardCollisionHandler extends StandardHandler {
	...
	public void tick() {
		...
		Rectangle cam = new Rectangle((int) (this.getCamera().getX() - vpo - this.getCamera().getVpw()),
				(int) (this.getCamera().getY() - this.getCamera().getVph()), this.getCamera().getVpw() * 2 + vpo * 2,
				this.getCamera().getVph() * 2);
					...
					for (int j = 0; j < this.getEntities().size(); j++) {
						...
						if (obj1 != obj2 && obj1.getId() != StandardID.Ignore && obj1.isAlive() && obj2.isAlive()) {
							if (StandardCollisionHandler.COLLISION_FLAGS.contains(obj2.getId())
								...
								if (StandardCollisionHandler.DEATH_FLAGS.contains(obj1.getId())) {
									obj1.setAlive(false);
								}
	...
	/**
	 * Adding an ID to this method will cause the collision handler to set the alive
	 * flag in the SGO to false if it comes across a collision with something.
	 *
	 * @param id
	 */
