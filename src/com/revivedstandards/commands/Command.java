...
public abstract class Command//And so we meet again Larru >:}
{
    ...
    {//expects any combination of the first 8 powers   of 2
        FOCUSED &= ~groups;
    }
    ...
    public static boolean deregister ( InputDevice id )
    {
        ...
        {//for each device
            if ( DEVICES[ i ] == id )
            {//if specified device is registered (if so, it will be the only instance)
    ...
    {
        if ( checkFlags( id, DID_0_OFF, DEVICE_MASK ) )
        {//Device id matches that in question?
        ...
        {//shift over the device and key values (see rebinding)
